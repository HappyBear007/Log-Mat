...
class SetHimpunan(Scene):
    def construct(self):
        ...
        irisan_B_label.next_to(irisan_A_label, DOWN).align_to(
            irisan_A_label, LEFT)
        ...
        self.play(Create(irisan_A_label), Create(
            irisan_B_label), Create(anb_label))
        self.wait(1)
        ...
        iris_label_3.move_to(
            (irisan_A.get_center() + irisan_B.get_center()) / 2 + UP * 0.5)
        ...
        iris_label_5.move_to(
            (irisan_A.get_center() + irisan_B.get_center()) / 2 + DOWN * 0.5)
        self.play(Create(iris_label_5))
        self.wait(0.5)

        self.play(
            FadeOut(Group(irisan_A, irisan_B, irisan_label, irisan_A_label, irisan_B_label, anb_label,
                    iris_label_A1, iris_label_A2, iris_label_B1, iris_label_B2, iris_label_3, iris_label_4, iris_label_5))
        )
        self.wait(1)
        ...
        elem_label_3.move_to(
            (circle_A.get_center() + circle_B.get_center()) / 2 + UP * 0.5)
        self.play(Create(elem_label_3))
        self.wait(0.5)

        elem_label_4 = Tex("4", color=WHITE).scale(0.7)
        elem_label_4.move_to(
            (circle_A.get_center() + circle_B.get_center()) / 2 + DOWN * 0.5)
        ...
        selisih_A_label.next_to(
            selisih_label, DOWN).align_to(selisih_label, LEFT)
        selisih_B_label = Tex("B = \\{2, 3, 5, 7, 11\\}", color=WHITE)
        selisih_B_label.next_to(selisih_A_label, DOWN).align_to(
            selisih_A_label, LEFT)
        ...
        self.play(Create(selisih_A_label), Create(
            selisih_B_label), Create(ab_label))
        self.wait(1)
        ...
        selisih_label_3.move_to(
            (selisih_A.get_center() + selisih_B.get_center()) / 2 + UP * 0.5)
        self.play(Create(selisih_label_3))
        self.wait(0.5)

        selisih_label_4 = Tex("3", color=WHITE).scale(0.7)
        selisih_label_4.move_to(
            (selisih_A.get_center() + selisih_B.get_center()))
        self.play(Create(selisih_label_4))
        self.wait(0.5)

        selisih_label_5 = Tex("5", color=WHITE).scale(0.7)
        selisih_label_5.move_to(
            (selisih_A.get_center() + selisih_B.get_center()) / 2 + DOWN * 0.5)
        self.play(Create(selisih_label_5))
        self.wait(0.5)

        self.play(
            FadeOut(Group(selisih_A, selisih_B, selisih_label, selisih_A_label, selisih_B_label, ab_label, selisih_label_A1,
                    selisih_label_A2, selisih_label_B1, selisih_label_B2, selisih_label_3, selisih_label_4, selisih_label_5))
        )
        self.wait(1)
        ...
        komp_A_label.next_to(komplemen_label, DOWN).align_to(
            komplemen_label, LEFT)
        ...
        # D. BEDA SETANGKUP
        bedset_A = Circle(radius=1.5, color=WHITE, fill_opacity=0.6)
        bedset_B = Circle(radius=1.5, color=RED, fill_opacity=0.6)

        bedset_A.shift(LEFT)
        bedset_B.shift(RIGHT)

        self.play(Create(bedset_A), Create(bedset_B))
        self.wait(1)

        bedset_label = Tex("Contoh Beda Setangkup", color=WHITE)
        bedset_label.to_edge(LEFT).to_edge(UP)
        bedsetlemen_label = Tex("A = \\{1, 2, 3, 4, 5\\}", color=WHITE)
        bedsetlemen_label.next_to(
            bedset_label, DOWN).align_to(bedset_label, LEFT)
        bedset_A_label = Tex("B = \\{2, 3, 5, 7, 11\\}", color=WHITE)
        bedset_A_label.next_to(bedsetlemen_label, DOWN).align_to(
            bedsetlemen_label, LEFT)
        bedset_B_label = MathTex(
            "A \\oplus B = \\{1, 4, 7, 11\\}", color=WHITE)
        bedset_B_label.next_to(bedset_A_label, DOWN).align_to(
            bedset_A_label, LEFT)

        self.play(Create(bedset_label), Create(bedsetlemen_label))
        self.wait(1)

        self.play(Create(bedset_A_label), Create(bedset_B_label))
        self.wait(1)

        # anggota himpunan A di dalam lingkaran A
        bedset_label_A1 = Tex("2", color=WHITE).scale(0.7)
        bedset_label_A1.move_to(bedset_A.get_center() + UP * 0.5)
        self.play(Create(bedset_label_A1))
        self.wait(0.5)

        bedset_label_A2 = Tex("3", color=WHITE).scale(0.7)
        bedset_label_A2.move_to(bedset_A.get_center())
        self.play(Create(bedset_label_A2))
        self.wait(0.5)

        bedset_label_A3 = Tex("5", color=WHITE).scale(0.7)
        bedset_label_A3.move_to(bedset_A.get_center() + DOWN * 0.5)
        self.play(Create(bedset_label_A3))
        self.wait(0.5)

        # anggota himpunan B di dalam lingkaran B
        bedset_label_B1 = Tex("2", color=WHITE).scale(0.7)
        bedset_label_B1.move_to(bedset_B.get_center() + UP * 0.5)
        self.play(Create(bedset_label_B1))
        self.wait(0.5)

        bedset_label_B2 = Tex("3", color=WHITE).scale(0.7)
        bedset_label_B2.move_to(bedset_B.get_center())
        self.play(Create(bedset_label_B2))
        self.wait(0.5)

        bedset_label_B3 = Tex("5", color=WHITE).scale(0.7)
        bedset_label_B3.move_to(bedset_B.get_center() + DOWN * 0.5)
        self.play(Create(bedset_label_B3))
        self.wait(0.5)

        # anggota himpunan yang ada di antara A dan B
        bedset_label_3 = Tex("1", color=WHITE).scale(0.7)
        bedset_label_3.move_to(
            (bedset_A.get_center() + bedset_B.get_center()) / 2 + UP * 0.5)
        self.play(Create(bedset_label_3))
        self.wait(0.5)

        bedset_label_4 = Tex("4", color=WHITE).scale(0.7)
        bedset_label_4.move_to((bedset_A.get_center() + bedset_B.get_center()))
        self.play(Create(bedset_label_4))
        self.wait(0.5)

        bedset_label_5 = Tex("7", color=WHITE).scale(0.7)
        bedset_label_5.move_to(
            (bedset_A.get_center() + bedset_B.get_center()) / 2 + DOWN * 0.5)
        self.play(Create(bedset_label_5))
        self.wait(0.5)

        bedset_label_6 = Tex("11", color=WHITE).scale(0.7)
        bedset_label_6.move_to(
            (bedset_A.get_center() + bedset_B.get_center()) / 2 + DOWN * 1.0)
        self.play(Create(bedset_label_6))
        self.wait(0.5)

        self.play(
            FadeOut(Group(bedset_A, bedset_B, bedset_label, bedsetlemen_label, bedset_A_label, bedset_B_label,
                          bedset_label_A1, bedset_label_A2, bedset_label_A3, bedset_label_B1, bedset_label_B2, bedset_label_B3, bedset_label_3, bedset_label_4, bedset_label_5, bedset_label_6))
        )
        self.wait(1)
        # BATAS BEDA SETANGKUP

        # E. PERKELIAN KARTESIAN
        perka_label = Tex("Contoh Perkalian Kartesian", color=WHITE)
        perka_label.to_edge(LEFT).to_edge(UP)
        perkalemen_label = Tex("A = \\{1, 2, 3\\}", color=WHITE)
        perkalemen_label.next_to(perka_label, DOWN).align_to(perka_label, LEFT)
        perka_A_label = Tex("B = \\{7, 9\\}", color=WHITE)
        perka_A_label.next_to(perkalemen_label, DOWN).align_to(
            perkalemen_label, LEFT)
        perka_B_label = Tex(
            "A X B = \\{(1,7), (1,9), (2,7), (2,9), (3,7), (3,9)\\}", color=WHITE)
        perka_B_label.next_to(perka_A_label, DOWN).align_to(
            perka_A_label, LEFT)

        self.play(Create(perka_label), Create(perkalemen_label))
        self.wait(1)

        self.play(Create(perka_A_label), Create(perka_B_label))
        self.wait(1)

        self.play(
            FadeOut(Group(perka_label, perkalemen_label,
                    perka_A_label, perka_B_label))
        )
        self.wait(1)
        # BATAS PERKALIAN KARTESIAN
